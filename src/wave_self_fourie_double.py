...
def model_wave(X, net):
    ...
    x_loc = x_loc.view(x_loc.shape[0], -1, 2).to(torch.complex64)
...
def model_wave_energy_simple(X, net, x_max = 10, fourier =True):
    ...
    E = torch.tensor([torch.sum(torch.abs(torch.abs(Xi)**2)) for Xi in X[0]])
...
X_train, y_train = wave_system.gen_wave_fourier_rand_fixed_speed(num = num_train, Nx = nx, x_max= L, tf=T)
X_test, y_test = wave_system.gen_wave_fourier_rand_fixed_speed(num=num_test, Nx = nx, x_max = L, tf = T)
...
net = dde.nn.DeepONetComplex(
    [2, 40, 40], [1, 80, 80, 80], complex_relu, "Glorot normal"
)  
...
X_test_fixed, y_test_fixed = wave_system.gen_wave_fourier_init_fixed_speed(num = 400, Nx = nx, x_max = L, tf = T)
...
y_pred_fixed_sol = ifft(y_pred_fixed.detach().numpy(), axis = 1)
y_test_fixed_sol = ifft(y_test_fixed, axis = 1)

wave_system.plot_wave_3d(y_pred_fixed_sol, y_test_fixed_sol, model,net, optimizer, x_max = L, T = T)
