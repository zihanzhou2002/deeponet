...
class FNNComplex(NN):
    """Fully-connected neural network, modified for complex-valued tensors."""
    ...
    def __init__(
        ...
        # Handle activation, supporting either list or single activation.
        if isinstance(activation, list):
            if not (len(layer_sizes) - 1) == len(activation):
                raise ValueError(
                    "Total number of activation functions do not match with sum of hidden layers and output layer!"
        ...
        for i in range(1, len(layer_sizes)):
            linear_layer = torch.nn.Linear(
                layer_sizes[i - 1], layer_sizes[i], dtype=torch.complex64
            )
            self.linears.append(linear_layer)

            # Initialize weights and biases (special handling if initializers aren't complex-aware)
            self._initialize_complex_weights(linear_layer, initializer, initializer_zero)

    def _initialize_complex_weights(self, layer, initializer, initializer_zero):
        """Initializes weights and biases for complex layers."""
        # Initialize real and imaginary parts separately, and combine.
        real_weight = torch.empty_like(layer.weight.real, dtype=torch.float32)
        imag_weight = torch.empty_like(layer.weight.imag, dtype=torch.float32)

        initializer(real_weight)
        initializer(imag_weight)

        # Combine into a complex weight.
        with torch.no_grad():
            layer.weight.copy_(real_weight + 1j * imag_weight)

        real_bias = torch.empty_like(layer.bias.real, dtype=torch.float32)
        imag_bias = torch.empty_like(layer.bias.imag, dtype=torch.float32)

        initializer_zero(real_bias)
        initializer_zero(imag_bias)

        with torch.no_grad():
            layer.bias.copy_(real_bias + 1j * imag_bias)

    def forward(self, inputs):
        x = inputs.to(torch.complex64)

        if self._input_transform is not None:
            x = self._input_transform(x).to(torch.complex64)

        # Process through hidden layers.
        for j, linear in enumerate(self.linears[:-1]):
            x = linear(x)

            # Complex activation functions (need to ensure your activations are complex-safe)
            if isinstance(self.activation, list):
        ...
        if self._output_transform is not None:
            x = self._output_transform(inputs, x).to(torch.complex64)
