...
from .fnn import FNN, FNNComplex
from .nn import NN
...
from ..deeponet_strategy import (
    ...
    SplitBranchStrategy,
    SplitTrunkStrategy,
    SplitBranchMatrixStrategy,
    SplitBranchEnergyStrategy
)
...
class DeepONet(NN):
    ...
    def __init__(
        ...
        self.multi_output_strategy = {
            ...
            "split_branch_matrix":SplitBranchMatrixStrategy,
            "split_branch_energy":SplitBranchEnergyStrategy
...
class DeepONetComplex(NN):
    """Deep operator network, modified for complex-valued inputs and operations."""
    ...
    def __init__(
        ...
        if isinstance(activation, dict):
            ...
            self.activation_branch = self.activation_trunk = activation #activations.get(activation)
        ...
        # Build networks using the strategy
        self.branch, self.trunk = self.multi_output_strategy.build(
            layer_sizes_branch, layer_sizes_trunk
        )
        if isinstance(self.branch, list):
        ...
        # Initialize complex-valued biases (b)
        self.b = torch.nn.ParameterList(
            [torch.nn.Parameter(torch.zeros(1, dtype=torch.complex64)) for _ in range(self.num_outputs)]
        )
    ...
    def build_branch_net(self, layer_sizes_branch):
        ...
        if callable(layer_sizes_branch[1]):
            return layer_sizes_branch[1]
        # Fully connected network (assumed to need adaptation for complex values)
        return FNNComplex(layer_sizes_branch, self.activation_branch, self.kernel_initializer)

    def build_trunk_net(self, layer_sizes_trunk):
        return FNNComplex(layer_sizes_trunk, self.activation_trunk, self.kernel_initializer)

    def merge_branch_trunk(self, x_func, x_loc, index):
        # Elementwise product and sum - both should be complex
        y = torch.einsum("bi,bi->b", x_func, x_loc).to(torch.complex64)
    ...
    @staticmethod
    def concatenate_outputs(ys):
        # Concatenating complex outputs
        return torch.cat(ys, dim=1).to(torch.complex64)

    def forward(self, inputs):
        x_func = inputs[0].to(torch.complex64)
        x_loc = inputs[1].to(torch.complex64)

        # Trunk net input transform
        if self._input_transform is not None:
            x_loc = self._input_transform(x_loc).to(torch.complex64)
